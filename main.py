    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                flash("syou are now logged in!", category="success")
...
def register():
    if request.method == 'POST':
        ...
        if len(username) < 5:
            ...
            create_user = Users(username=username, email=email, first_name=first_name, last_name=last_name, password=generate_password_hash(password1, method="sha256"))
